...
export function Profile() {
  ...
  const handleUpdateProfile = async () => {
    ...
    setSuccess(null)
    try {
      const res = await api.put('/user/fullname', { fullname: formData.fullname })
      if (res.status === 200) {
  ...
  const handleImageUpload = async (e) => {
    ...
    setError(null)
    try {
      const res = await api.post('/user/profile-pic', fd, {
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }} className="space-y-6">
      ...
      <div className="rounded-2xl p-6"
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
          <div className="relative">
            <div className="w-24 h-24 rounded-2xl overflow-hidden"
              ...
              {formData.profilePic ? (
                <img src={formData.profilePic} alt={formData.fullname} className="w-full h-full object-cover" />
              ) : (
            ...
            {/* Profile pic upload — available for ALL users (email + github) */}
            <label
              ...
              style={{ background: '#00e5ff', boxShadow: '0 4px 12px rgba(0,229,255,0.3)' }}>
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: '#6b7280' }}>Member since {formatDate(user?.createdAt)}</p>
            {isGithubUser && (
              <p className="text-xs mt-2 flex items-center gap-1.5" style={{ color: '#9ca3af' }}>
                ...
                Name & email managed by GitHub.
              </p>
            )}
          </div>
        </div>

        {/* Profile fields — GitHub users see read-only name+email, no edit button */}
        <div className="space-y-4">
          ...
          {/* Edit name button — email users only */}
          {!isGithubUser && (
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              <div className="p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)' }}>
                ...
                <ul className="text-xs space-y-1" style={{ color: '#6b7280' }}>
                  {[
                    { label: 'At least 6 characters', check: passwordData.newPassword.length >= 6 },
                    { label: 'At least one uppercase letter', check: /[A-Z]/.test(passwordData.newPassword) },
                    { label: 'At least one number', check: /[0-9]/.test(passwordData.newPassword) },
                  ].map(r => (
                    <li key={r.label} className="flex items-center gap-2">
                      <span className={`w-1 h-1 rounded-full ${r.check ? 'bg-green-400' : 'bg-gray-500'}`} />
                      {r.label}
                    </li>
                  ))}
                </ul>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="px-4 py-2.5 rounded-xl font-bold text-sm text-black transition-all hover:scale-105 disabled:opacity-50 disabled:hover:scale-100"
                  style={{
                    background: 'linear-gradient(135deg, #00e5ff, #00b8cc)',
                    boxShadow: '0 4px 16px rgba(0,229,255,0.2)'
                  }}>
      ...
      {/* Account Stats — no plan/subscription info */}
      <div className="rounded-2xl p-6"
        style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)' }}>
            ...
            <p className="text-sm font-syne font-bold text-white">{formatDate(user?.updatedAt)}</p>
          </div>
        </div>
      </div>
